...
def plotScatterTwoTheta(pickleFileName, trueValues, stages, labelsName, thetaChoice):
    ...
    for i in stages:
        ...
        if i == 0:
            ...
            betap = mytrace[i - 1][-2]
        plt.scatter(Sm[:, thetaChoice[0]], Sm[:, thetaChoice[1]], s=(Wm / sum(Wm)) * 1000, zorder=2)
        plt.xlim([0.01 * (trueValues[1]), 2.2 * (trueValues[1])])
        plt.ylim([0.01 * (trueValues[0]), 2.2 * (trueValues[0])])
